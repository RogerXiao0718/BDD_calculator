...
@when(parsers.parse('I enter "{expression}" first'))
def enter_expression1(browser, operation_cache, expression):
...
@when(parsers.parse('I enter "{expression}" again'))
def click_equal_button(browser, operation_cache, expression):
    expression_input = browser.find_element_by_name('expression')
    expression_input.send_keys(expression)
    browser.find_element_by_name('calculate').click()
    browser_answer = browser.find_element_by_id('ans-txt').get_attribute('innerHTML')
